...
```text
/plugin marketplace add fact0-ai/claude-code-plugin
...
Privacy-first by default: prompts and tool inputs/outputs are captured as a
SHA-256 hash plus length/summary metadata, and redaction happens **on your
machine before anything is sent**. Set `FACT0_CC_CAPTURE_RAW=1` to opt in to raw
text capture where that is acceptable.
...
| `FACT0_CC_CAPTURE_RAW` | off | Include raw prompt/tool text in addition to hashes. |
