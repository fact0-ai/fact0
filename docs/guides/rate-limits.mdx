...
| 400, 401, 403, 409, 413 | Fail immediately; repair input/configuration before retry |
...
## Request and capture limits

The core limits encoded request bodies to 4 MiB. HTTP 200 may include per-item errors; inspect accepted/rejected counts. The Claude collector splits batches and retains permanent oversized failures locally within its configured spool limit. The core uses synchronous ingestion; receipt polling is unavailable.
