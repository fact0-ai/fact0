---
title: Audit client
description: Python AuditClient API reference for self-hosted audit records.
---

# Audit client

Writing events requires a write-scoped instance key. Read-scoped keys can query, verify and export records. The supported core commits ingestion synchronously; receipt polling methods below remain SDK compatibility APIs and are unavailable on this core.
...
| `base_url` | `https://api.fact0.io` | Compatibility default; explicitly set your self-hosted origin as above |
...
async def lifespan(app: FastAPI):
    ...
    app.state.fact0 = fact0.AsyncClient(api_key=os.environ['FACT0_API_KEY'], sync=False, base_url=os.environ.get('FACT0_BASE_URL', 'http://localhost:8000'))
    yield
...
def replay_dead_letters(file_path: str, api_key: str):
    client = fact0.Client(api_key=api_key, sync=True, base_url=os.environ.get('FACT0_BASE_URL', 'http://localhost:8000')) # Use sync ingest for verification
    replayed_count = 0
    events = []
